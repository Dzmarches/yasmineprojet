...
const Typerevenue = () => {
    ...
    const AjouterModifierTR = async () => {
        ...
        try {
            ...
            let response;
            if (isEditMode) {
                response = await axios.put(`http://localhost:5000/Typerevenus/modifier/${editId}`, formData, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json",
                    },
                ...
                response = await axios.post('http://localhost:5000/Typerevenus/ajouter', formData, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json",
                    },
    ...
    const ArchiverHS = async (id) => {
        try {
            ...
            const response = await axios.patch(`http://localhost:5000/Typerevenus/archiver/${id}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                },
    ...
    return (
        <>
            {/* <nav>
                ...
                <span> / </span>
                <span>Gestion des types de  revenus</span>
            </nav> */}
                                                        ...
                                                        <div className="card-body border-0 rounded-lg">
                                                            <div className="row">
                                                                <div className="col-md-4">
                                                                    <label>Type *</label>
                                                                    <input type="text" className="form-control" name="type" value={formData.type} 
                                                                    style={{ height: "40px" }} onChange={handleChange} />
                                                                    {errors.type && <span className="text-danger">{errors.type}</span>}
                                                                </div>
                                                                <div className="col-md-4">
                                                                    <label>Remarque </label>
                                                                    <input type="text" className="form-control" name="remarque" value={formData.remarque} 
                                                                    style={{ height: "40px" }} onChange={handleChange} />
                                                        ...
                                                        <tbody>
                                                            {currentItems.map((item, index) => (
                                                                <tr key={index}>
                                                                    <td>{index+1}</td>
                                                                    <td>{item.type}</td>
                                                                    <td style={styleRemarque}>{item.remarque}</td>
