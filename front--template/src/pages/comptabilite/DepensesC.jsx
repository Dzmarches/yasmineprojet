...
const DepensesC = () => {
  ...
  useEffect(() => {
    handleListTypeDepenses();
    Listedepenses();
  }, []);

  const handleListTypeDepenses = async () => {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      setTypedepense(TROptions);
    } catch (error) {
      console.error('Erreur lors de la récupération les types des depenses', error);
      alert('Une erreur est survenue lors de la récupération les types des depenses');
    }
  };
  ...
  const Listedepenses = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/depenses/liste/`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const filteredData = data.filter(item => {
    ...
    const matchesSearchTerm = search === '' || (
      ...
      (item.TypeDepense?.type && item.TypeDepense.type.includes(searchTerm)) ||
            ...
            <tbody>
              ${currentItems.map(item => `
                <tr>
                  <td>${item.id}</td>
                  <td>${item.code || ''}</td>
                  <td>${item.TypeDepense ? item.TypeDepense.type : ""}</td>
                  <td>${item.cause_ar || ''}</td>
                  <td>${item.cause_fr || ''}</td>
                  <td>${item.montant || ''}</td>
  ...
  const Archiverdepense = async (id) => {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
        alert("Vous devez être connecté");
      ...
      const response = await axios.patch(`http://localhost:5000/depenses/archiver/${id}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.type) newErrors.type = "Le type de depense est requis";
  ...
  const Ajouterdepense = async () => {
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(`http://localhost:5000/depenses/modifier/${editId}`, formDataToSend, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
        });
        alert('depense modifié avec succès');
      } else {
        response = await axios.post('http://localhost:5000/depenses/ajouter', formDataToSend, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
                          ...
                          <div className="card-body border-0 rounded-lg">
                            <div className="row">
                              ...
                              <div className="col-md-12 mt-3">
                                <button type="button" className="btn btn-outline-primary" onClick={Ajouterdepense}>
          ...
          <Modal show={showDeleteModal} onHide={handleClose}>
            ...
            <Modal.Footer>
              ...
              <Button
                variant="danger"
                onClick={() => {
                  Archiverdepense(depenseIdToDelete);
                  handleClose();
                }}
              >
                Archiver
